 ...
 * 一次性执行任务，执行完成结束
 *
 * ScheduledExecutorService： scheduleAtFixedRate（Runnable command, long initialDelay, long period,
 * TimeUnit unit）
 ...
 * @ TimeUnit unit
 *
 * 包含首次延迟的周期性执行任务，第一次执行：delay+period，第二次：delay+2*period,以此类推... 停止：异常停止执行，主动调用停止方法
 ...
 * @ unit：前两个参数的单位
 *
 * 周期性执行任务：第一次执行：initialDelay+delay，第二次：initialDelay+2*delay,以此类推... 停止：异常停止执行，主动调用停止方法 不顺延
 *
 */
...
//    es.schedule(mer, 1000, TimeUnit.SECONDS.MILLISECONDS);
    es.scheduleAtFixedRate(mer, 2, 4, TimeUnit.SECONDS);
